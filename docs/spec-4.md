...
Status: round 4, ready for review. No open questions are left. At the user's request, the questions left in [spec-3.md](spec-3.md) were decided by inference from earlier answers and standard Wordle features.
...
- Hint words do not have to follow hard mode rules. Letters they reveal do count for the rules on later guesses [inferred: a hint is help from the game, not a player guess].
...
| 2 | Hints and hard mode? | Hint words are exempt. Their letters count afterwards | A hint is help, not a guess |
