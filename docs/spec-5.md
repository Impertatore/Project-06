...
Status: round 5. Same as [spec-4.md](spec-4.md), except that hints now keep what the player has found (section 3.7), as the user asked after the first build. No open questions are left. The questions left in [spec-3.md](spec-3.md) were decided by inference in round 4, at the user's request.
...
- The player gets 2 hints per game. A hint fills the next empty row with a word that shares at least one letter with the answer, and keeps every letter found so far.
...
- So a hint never loses what the player has already found. These are the same rules as hard mode (3.10), and they apply to hints whether hard mode is on or off.
- Letters a hint reveals also count for hard mode rules on later guesses.
- If no word other than the answer fits, pressing "Hint" shows "No hint available: only the answer fits what you have found". No hint is used up.
...
| CRANE | SLATE ⬜⬜🟩⬜🟩 | STAKE | Yes | Keeps A 3rd and E 5th |
| CRANE | SLATE ⬜⬜🟩⬜🟩 | TRAIN | No | Loses E in 5th place |
| CRANE | TRAIN ⬜🟩🟩⬜🟨 | PRANK | Yes | Keeps R and A in place, uses N |
| CRANE | TRAIN ⬜🟩🟩⬜🟨 | BRAND | Yes | Keeps R and A in place. N lands in its right place by chance |
...
| Answer CRANE, row 1 is CRANK, "Hint (2)" | Yes | "No hint available: only the answer fits what you have found". Still "Hint (2)" |
...
42a. An automated test checks every row of the first table in 3.7, including that a hint keeps green letters from earlier rows in place, uses their yellow letters, and is not given when only the answer fits.
...
| 2 | Hints and hard mode? | **Changed in round 5 by the user:** hints always keep green letters in place and use yellow letters (the hard mode rules), whether hard mode is on or off. Their letters count afterwards | "We should not lose the existing finds with the hint" |
