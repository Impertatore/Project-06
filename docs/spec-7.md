...
> **Round 7. Not yet promoted until this file is copied over `spec-current.md`.**
> This is the spec the agents and skills read. To change it, author a new
> round as `docs/spec-N.md`, get it approved, then copy it over this file.
...
Status: round 7. Same as [spec-6.md](spec-6.md), except that the player now gets 3 hints per game instead of 2 (sections 1, 3.1, 3.2, 3.7, 3.11 and the criteria that name a hint count). Hints themselves work exactly as in round 6. Rounds 5 and 6 changed what a hint word must do; round 7 changes only how many the player gets. The questions left in [spec-3.md](spec-3.md) were decided by inference in round 4, at the user's request. The open questions this change raised are in [docs/changes/three-hints-per-game.md](changes/three-hints-per-game.md).
...
- The player gets 3 hints per game. A hint fills the next empty row with a word that shares at least one letter with the answer, keeps every letter found so far, and finds something new.
...
- Header, top-right, in this order: "Hint (3)", "Stats", "New Game", "Help", "Settings".
...
- A new game clears the grid and the key colours, and resets the hint button to "Hint (3)" [assumed].
...
- The player has 3 hints per game.
- The hint button shows how many are left: "Hint (3)", "Hint (2)", "Hint (1)", "Hint (0)".
- All 3 can be used in one game: hints are allowed on rows 1 to 5, so 3 hints always fit [assumed].
...
|---|---|---|
| Row 1, empty, "Hint (3)" | Yes | Row 1 gets a hint word, coloured. "Hint (2)" |
| Row 2, empty, "Hint (2)" | Yes | Row 2 gets a hint word, coloured. "Hint (1)" |
| Row 3 has `CR` typed, "Hint (1)" | Yes | `CR` is replaced by a hint word, coloured. "Hint (0)" |
| Three hints used, game still in play | Yes | Nothing. Button is disabled at "Hint (0)" |
| "Hint (0)" | Yes | Nothing. Button is disabled |
| Rows 1–5 used, "Hint (2)" | Yes | Nothing. Button is disabled |
| Answer CRANE, row 1 is CRANK, "Hint (3)" | Yes | "No hint available: no word other than the answer keeps your finds and adds a new one". Still "Hint (3)" |
...
- Statistics shown:
  ...
  - hints per game: the average number of hints used per game played, to one decimal place [inferred: "per game" read as an average, like win percentage]. It can reach 3.0.
  - Statistics saved before round 7 are kept as they are. The average then mixes games played with 2 hints available and games played with 3 [assumed].
...
- It says the player gets 3 hints per game [assumed: the count is shown to the player, so the help must match the button].
...
11. Manual step: the header shows "Hint (3)", "Stats", "New Game", "Help" and "Settings" at the top-right, in that order.
...
35. Manual step: after "New Game", the grid and key colours are cleared and the button shows "Hint (3)".
...
41. Manual step: press "Hint" three times in one game. Three different words fill three rows, and the button reads "Hint (3)", "Hint (2)", "Hint (1)", "Hint (0)" in turn [assumed: the two-hint version of this step no longer reaches the limit].
41a. Manual step: at "Hint (0)", with the game still in play, the hint button is disabled and looks disabled, and pressing it does nothing.
41b. An automated test checks that a new game has 3 hints, that each hint used leaves one fewer, and that a fourth press in one game uses no row.
...
42c. Manual step, known answer `CRANE`: guess `CRANK`, then press "Hint". The "No hint available" message shows, no row is filled, and the button still shows "Hint (3)".
...
48. Manual step: reset statistics, then play one game using 3 hints and one using none. Hints per game shows 1.5 [assumed: the 2-hint/1.0 version of this step no longer uses the maximum].
48a. Manual step: with statistics saved before round 7, load the app after the change. The statistics are readable and unchanged; nothing is reset.
...
59. Manual step: "Help" opens a panel that explains colours, hints, hard mode and quitting, and says the player gets 3 hints per game.
...
None in this file. At the user's request, every question left in [spec-3.md](spec-3.md) was decided by inference. The table lists each decision so it can be changed. The open questions raised by the round 7 change are in [docs/changes/three-hints-per-game.md](changes/three-hints-per-game.md) and are for the product-owner.
...
| 1d | Hints "per game" | Average hints per game played, up to 3.0 | Other statistics are totals and averages |
...
| 2 | How many hints per game? | **Changed in round 7 by the user:** 3, not 2 | "change the number of hints per game from 2 to 3" |
