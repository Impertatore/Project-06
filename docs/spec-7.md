...
Status: round 7. Same as [spec-6.md](spec-6.md), except that a game now gives 3 hints instead of 2 (sections 1, 3.1, 3.2, 3.7, 3.11 and the criteria that name the count). Rounds 5 and 6 changed how a hint is chosen; round 7 changes only how many there are. The hint rules themselves are unchanged. Open questions for this round are listed in [docs/changes/three-hints-per-game.md](changes/three-hints-per-game.md); where they are still open, the text below records the assumption made.
...
| Row 3 has `CR` typed, "Hint (2)" | Yes | `CR` is replaced by a hint word, coloured. "Hint (1)" |
| Row 4, empty, "Hint (1)" | Yes | Row 4 gets a hint word, coloured. "Hint (0)". Button becomes disabled |
...
- It states how many hints a game gives: 3 [assumed: text shown to players must match the rules].
...
41. Manual step: press "Hint" three times in one game. Three different words fill three rows, and the button counts down "Hint (2)", "Hint (1)", "Hint (0)". If no word qualifies at some press, the "No hint available" message shows instead and that hint is not used up.
...
None in the spec itself. At the user's request, every question left in [spec-3.md](spec-3.md) was decided by inference. The table lists each decision so it can be changed. Round 7's own open questions are in [docs/changes/three-hints-per-game.md](changes/three-hints-per-game.md); the text above records the assumption made for each one.
...
| — | How many hints per game? | **Changed in round 7 by the user:** 3, not 2. The hint rules, the disabled conditions and the statistics are unchanged | "change the number of hints per game from 2 to 3" |
