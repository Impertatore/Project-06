...
> **Round 7.** Authored as `docs/spec-7.md`. Once approved it is copied over
> `docs/spec-current.md`, which is the file the agents and skills read.
...
Status: round 7. Same as [spec-6.md](spec-6.md), except that a game now gives 3 hints instead of 2 (sections 1, 3.1, 3.2, 3.7 and the criteria that name a hint count). Rounds 5 and 6 changed which words a hint may use; round 7 changes only how many hints a player gets. No open questions are left. The questions left in [spec-3.md](spec-3.md) were decided by inference in round 4, at the user's request.
...
| Rows 1–3 are hints, rows 4–6 free, "Hint (0)" | Yes | Nothing. Button is disabled |
...
41. Manual step: press "Hint" three times in one game. Three different words fill three rows, and the button reads "Hint (2)", "Hint (1)" and "Hint (0)" in turn.
41a. Manual step: after three hints on rows 1–3, rows 4–6 are still free and the hint button is disabled showing "Hint (0)".
...
| 1d | Hints "per game" | Average hints per game played. **Round 7:** the cap rose to 3, so the highest possible average is 3.0. Statistics saved before round 7 are kept as they are, so the average can span games played under both caps | Other statistics are totals and averages; player data is never rewritten |
...
| 2 | Hint on the last row? | Not allowed. Button disabled. **Round 7:** unchanged with 3 hints, so all 3 can be used on rows 1–5 | A hint could otherwise lose the game |
