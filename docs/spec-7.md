...
Status: round 7. Same as [spec-6.md](spec-6.md), except for the number of hints per game, which goes from 2 to 3 (sections 1, 3.1, 3.2, 3.7 and the criteria that name the count). The hint rules themselves are unchanged. Rounds 5 and 6 changed how a hint is chosen; round 7 changes only how many the player gets. No open questions are left. The questions left in [spec-3.md](spec-3.md) were decided by inference in round 4, at the user's request.
...
- All 3 hints fit in one game: up to 5 rows may hold a hint. A player who uses all 3 has 3 rows left to guess in, so the best win message they can reach is "Splendid" (4 rows used).
...
- Statistics shown:
  ...
  - hints per game: the average number of hints used per game played, to one decimal place [inferred: "per game" read as an average, like win percentage]. Up to 3.0 for games played under this round. Games played when the allowance was 2 stay in the totals as they are; nothing is reset [assumed].
...
41. Manual step: press "Hint" three times in one game. Three different words fill three rows, and the button reads "Hint (2)", "Hint (1)", "Hint (0)" in turn, then is disabled.
...
43. Manual step: after a win, loss or quit, the hint button is disabled, even with hints left.
43a. An automated test checks that a new game has 3 hints, that three hints can be used in one game, and that a fourth press does nothing.
43b. Manual step, known answer `CRANE`: use three hints, then guess `CRANE` on row 4. The win message is "Splendid".
...
48. Manual step: reset statistics, then play one game using 3 hints and one using none. Hints per game shows 1.5.
...
| — | How many hints per game? | **Changed in round 7 by the user:** 3, not the 2 in the notes. The hint rules and the last-row limit are unchanged, and no saved statistic is reset | "change the number of hints per game from 2 to 3" |
